  ...
  try {
    ...
    try {
      const productRes = await fetchWithTimeout(
        ...
        { 
          next: { revalidate: 60 },
          timeout: 3023 // 3 seconds timeout for product API
        }
      );
    ...
    try {
      const categoryRes = await fetchWithTimeout(
        ...
        { 
          next: { revalidate: 60 },
          timeout: 3023 // 3 seconds timeout for category API
        }
      );
    ...
    try {
      const newsRes = await fetchWithTimeout(
        ...
        { 
          next: { revalidate: 60 },
          timeout: 3023 // 3 seconds timeout for news API
        }
      );
    ...
    try {
      const newsCategoryRes = await fetchWithTimeout(
        ...
        { 
          next: { revalidate: 60 },
          timeout: 3023 // 3 seconds timeout for news category API
        }
      );
