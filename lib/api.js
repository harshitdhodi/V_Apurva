// src/lib/api.js
const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3058';
  ...
  try {
    ...
    try {
      const productRes = await fetchWithTimeout(
        ...
        { 
          next: { revalidate: 60 },
          timeout: 3058 // 3 seconds timeout for product API
        }
      );
    ...
    try {
      const categoryRes = await fetchWithTimeout(
        ...
        { 
          ...
          timeout: 3058 // 3 seconds timeout for category API
        }
      );
    ...
    try {
      const newsRes = await fetchWithTimeout(
        ...
        { 
          next: { revalidate: 60 },
          timeout: 3058 // 3 seconds timeout for news API
        }
      );
    ...
    try {
      const newsCategoryRes = await fetchWithTimeout(
        ...
        { 
          next: { revalidate: 3600 },
          timeout: 3058 // 3 seconds timeout for news category API
        }
      );
