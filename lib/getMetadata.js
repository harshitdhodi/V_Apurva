const baseUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3023' || "https://apurva.chemicals.com";
...
/**
 * Helper to fetch metadata from a given endpoint
 */
async function fetchMetaDataFromEndpoint(endpoint, slug, favicon) {
  const res = await fetch(`${baseUrl}${endpoint}?slug=${slug}`, {
    cache: 'no-store',
  });
  ...
  if (!res.ok) {
    ...
    return {
      ...
      alternates: { canonical: `https://apurvachemicals.com/${slug}` }
    };
  }
  ...
  return {
    ...
    icons: { icon: favicon || '/favicon.ico' },
    alternates: {
      canonical: meta.metaCanonical || `https://apurvachemicals.com/${slug}`,
    }
  };
}

/**
 * Main function to get metadata by slug.
 * For static pages, it will fetch from /api/staticMeta/metaBySlug
 */
export async function getMetadataBySlug(slug, isStatic = false) {
  try {
    const favicon = await getFaviconPath();
    const endpoint = isStatic
      ? '/api/staticMeta/metaBySlug'
      : '/api/meta';
