...
export async function GET(request, { params }) {
  try {
    ...
    const possiblePaths = [
      ...
      path.join(process.cwd(), 'app', 'api', 'logo', 'download', 'logos', filename)
    ];
    ...
    if (!fileExists) {
      console.error(`Logo not found at any location: ${filename}`);
    ...
    try {
      ...
      return new NextResponse('Error reading logo file', {
        status: 500,
        headers: {
    ...
    console.error('Unexpected error in logo download:', error);
